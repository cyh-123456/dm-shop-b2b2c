	...
	[#escape x as x?js_string]
		...
		+function($) {
			...
			CaptchaImage.prototype.init = function() {
				...
				this.$element.on("click.shopec.captchaImage", $.proxy(this.refresh, this));
			};
			...
			CaptchaImage.prototype.refresh = function() {
				var refreshEvent = $.Event("refresh.shopec.captchaImage");
				...
				this.$element.trigger("refreshed.shopec.captchaImage");
			};
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.captchaImage");
					var options = $.extend({}, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.captchaImage", (data = new CaptchaImage(this, options)));
					}
		...
		+function($) {
			...
			ScrollLoad.prototype.load = function() {
				...
				var loadEvent = $.Event("load.shopec.scrollLoad");
				...
				$.ajax({
					...
					that.$element.trigger("loaded.shopec.scrollLoad", arguments);
				}).fail(function() {
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.scrollLoad");
					var options = $.extend({}, ScrollLoad.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.scrollLoad", (data = new ScrollLoad(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
		...
		+function($) {
			...
			var AddCart = function(element, options) {
				...
				this.$element.on("click.shopec.addCart", $.proxy(this.execute, this));
			};
			...
			AddCart.prototype.execute = function() {
				var that = this;
				var beforeEvent = $.Event("before.shopec.addCart");
				...
				this.ajax().done(function() {
					...
					that.fly(function() {
						that.$element.trigger("success.shopec.addCart", ajaxArguments);
						that.$element.trigger("complete.shopec.addCart", ajaxArguments);
					});
				}).fail(function() {
					that.$element.trigger("error.shopec.addCart", arguments);
					that.$element.trigger("complete.shopec.addCart", arguments);
				});
			};
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.addCart");
					var options = $.extend({}, AddCart.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.addCart", (data = new AddCart(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.shopec.addCart.data-api", "[data-action='addCart']", function() {
				var $this = $(this);
				
				if ($this.data("shopec.addCart") != null) {
					return;
				}
		...
		+function($) {
			...
			ModifyCart.prototype.changing = function(event, newValue, oldValue) {
				var that = this;
				var beforeEvent = $.Event("before.shopec.modifyCart");
				...
				this.ajax(newValue).done(function() {
					that.$element.trigger("success.shopec.modifyCart", arguments);
				}).fail(function() {
					that.$element.val(oldValue);
					that.$element.trigger("error.shopec.modifyCart", arguments);
				}).always(function() {
					that.$element.trigger("complete.shopec.modifyCart", arguments);
				});
			};
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.modifyCart");
					var options = $.extend({}, ModifyCart.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.modifyCart", (data = new ModifyCart(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
		...
		+function($) {
			...
			var RemoveCart = function(element, options) {
				...
				this.$element.on("click.shopec.removeCart", $.proxy(this.execute, this));
			};
			...
			RemoveCart.prototype.execute = function() {
				...
				var beforeEvent = $.Event("before.shopec.removeCart");
				...
				if (confirm) {
					this.confirm(function(result) {
						...
						that.ajax().done(function() {
							that.$element.trigger("success.shopec.removeCart", arguments);
						}).fail(function() {
							that.$element.trigger("error.shopec.removeCart", arguments);
						}).always(function() {
							that.$element.trigger("complete.shopec.removeCart", arguments);
						});
					});
				} else {
					this.ajax().done(function() {
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.removeCart");
					var options = $.extend({}, RemoveCart.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.removeCart", (data = new RemoveCart(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.shopec.removeCart.data-api", "[data-action='removeCart']", function() {
				var $this = $(this);
				
				if ($this.data("shopec.removeCart") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var ClearCart = function(element, options) {
				...
				this.$element.on("click.shopec.clearCart", $.proxy(this.execute, this));
			};
			...
			ClearCart.prototype.execute = function() {
				var that = this;
				var confirm = typeof this.options.confirm === "function" ? this.options.confirm(this.$element) : this.options.confirm;
				var beforeEvent = $.Event("before.shopec.clearCart");
				...
				if (confirm) {
					this.confirm(function(result) {
						...
						that.ajax().done(function() {
							that.$element.trigger("success.shopec.clearCart", arguments);
						}).fail(function() {
							that.$element.trigger("error.shopec.clearCart", arguments);
						}).always(function() {
							that.$element.trigger("complete.shopec.clearCart", arguments);
						});
					});
				} else {
					this.ajax().done(function() {
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.clearCart");
					var options = $.extend({}, ClearCart.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.clearCart", (data = new ClearCart(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.shopec.clearCart.data-api", "[data-action='clearCart']", function() {
				var $this = $(this);
				
				if ($this.data("shopec.clearCart") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var Checkout = function(element, options) {
				...
				this.$element.on("click.shopec.checkout", $.proxy(this.execute, this));
			};
			...
			Checkout.prototype.execute = function() {
				...
				var skuId = typeof this.options.skuId === "function" ? this.options.skuId(this.$element) : this.options.skuId;
				var quantity = typeof this.options.quantity === "function" ? this.options.quantity(this.$element) : this.options.quantity;
				var beforeEvent = $.Event("before.shopec.checkout");
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.checkout");
					var options = $.extend({}, Checkout.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.checkout", (data = new Checkout(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.shopec.checkout.data-api", "[data-action='checkout']", function() {
				var $this = $(this);
				
				if ($this.data("shopec.checkout") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var AddProductFavorite = function(element, options) {
				...
				this.$element.on("click.shopec.addProductFavorite", $.proxy(this.execute, this));
			};
			...
			AddProductFavorite.prototype.execute = function() {
				var that = this;
				var beforeEvent = $.Event("before.shopec.addProductFavorite");
				...
				this.ajax().done(function() {
					...
					that.shrinkOut(function() {
						that.$element.trigger("success.shopec.addProductFavorite", ajaxArguments);
						that.$element.trigger("complete.shopec.addProductFavorite", ajaxArguments);
					});
				}).fail(function() {
					that.$element.trigger("error.shopec.addProductFavorite", arguments);
					that.$element.trigger("complete.shopec.addProductFavorite", arguments);
				});
			};
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.addProductFavorite");
					var options = $.extend({}, AddProductFavorite.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.addProductFavorite", (data = new AddProductFavorite(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.shopec.addProductFavorite.data-api", "[data-action='addProductFavorite']", function() {
				var $this = $(this);
				
				if ($this.data("shopec.addProductFavorite") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var AddStoreFavorite = function(element, options) {
				...
				this.$element.on("click.shopec.addStoreFavorite", $.proxy(this.execute, this));
			};
			...
			AddStoreFavorite.prototype.execute = function() {
				var that = this;
				var beforeEvent = $.Event("before.shopec.addStoreFavorite");
				...
				this.ajax().done(function() {
					...
					that.shrinkOut(function() {
						that.$element.trigger("success.shopec.addStoreFavorite", ajaxArguments);
						that.$element.trigger("complete.shopec.addStoreFavorite", ajaxArguments);
					});
				}).fail(function() {
					that.$element.trigger("error.shopec.addStoreFavorite", arguments);
					that.$element.trigger("complete.shopec.addStoreFavorite", arguments);
				});
			};
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.addStoreFavorite");
					var options = $.extend({}, AddStoreFavorite.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.addStoreFavorite", (data = new AddStoreFavorite(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.shopec.addStoreFavorite.data-api", "[data-action='addStoreFavorite']", function() {
				var $this = $(this);
				
				if ($this.data("shopec.addStoreFavorite") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var Delete = function(element, options) {
				...
				this.$element.on("click.shopec.delete", $.proxy(this.execute, this));
			};
			...
			Delete.prototype.execute = function() {
				var that = this;
				var confirm = typeof this.options.confirm === "function" ? this.options.confirm(this.$element) : this.options.confirm;
				var beforeEvent = $.Event("before.shopec.delete");
				...
				if (confirm) {
					this.confirm(function(result) {
						...
						that.ajax().done(function() {
							that.$element.trigger("success.shopec.delete", arguments);
						}).fail(function() {
							that.$element.trigger("error.shopec.delete", arguments);
						}).always(function() {
							that.$element.trigger("complete.shopec.delete", arguments);
						});
					});
				} else {
					this.ajax().done(function() {
			...
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.delete");
					var options = $.extend({}, Delete.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.delete", (data = new Delete(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.shopec.delete.data-api", "[data-action='delete']", function() {
				var $this = $(this);
				
				if ($this.data("shopec.delete") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var Refresh = function(element, options) {
				...
				this.$element.on("click.shopec.refresh", $.proxy(this.execute, this));
			};
			...
			Refresh.prototype.execute = function() {
				...
				var beforeEvent = $.Event("before.shopec.refresh");
				...
				this.$element.trigger("complete.shopec.refresh");
			};
			
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.refresh");
					var options = $.extend({}, Refresh.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.refresh", (data = new Refresh(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.shopec.refresh.data-api", "[data-action='refresh']", function() {
				var $this = $(this);
				
				if ($this.data("shopec.refresh") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		+function($) {
			...
			var CheckAll = function(element, options) {
				...
				this.$element.on("change.shopec.checkAll", $.proxy(this.toggle, this));
			};
			...
			CheckAll.prototype.check = function() {
				...
				var checkEvent = $.Event("check.shopec.checkAll");
				...
				this.$element.trigger("checked.shopec.checkAll");
			};
			
			CheckAll.prototype.uncheck = function() {
				var target = typeof this.options.target === "function" ? this.options.target(this.$element) : this.options.target;
				var $target = target instanceof jQuery ? target : $(target);
				var uncheckEvent = $.Event("uncheck.shopec.checkAll");
				...
				this.$element.trigger("unchecked.shopec.checkAll");
			};
			
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.checkAll");
					var options = $.extend({}, CheckAll.DEFAULTS, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.checkAll", (data = new CheckAll(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("change.shopec.checkAll.data-api", "[data-toggle='checkAll']", function() {
				var $this = $(this);
				
				if ($this.data("shopec.checkAll") != null) {
					return;
				}
		...
		+function($) {
			...
			var Back = function(element, options) {
				...
				this.$element.on("click.shopec.back", $.proxy(this.execute, this));
			};
			
			Back.prototype.execute = function() {
				var beforeEvent = $.Event("before.shopec.back");
				...
				this.$element.trigger("complete.shopec.back");
			};
			
			function Plugin(option) {
				return this.each(function() {
					var $this = $(this);
					var data = $this.data("shopec.back");
					var options = $.extend({}, $this.data(), typeof option === "object" && option);
					
					if (data == null) {
						$this.data("shopec.back", (data = new Back(this, options)));
					}
					if (typeof option === "string") {
						data[option]();
					}
			...
			$(document).on("click.shopec.back.data-api", "[data-action='back']", function() {
				var $this = $(this);
				
				if ($this.data("shopec.back") != null) {
					return;
				}
				Plugin.call($this, "execute");
			});
		...
		(function($) {
			...
			$.fn.ajaxSubmit.defaults = {
				...
				beforeSubmit: function(arr, $form, options) {
					var beforeEvent = $.Event("before.shopec.ajaxSubmit", arguments);
				...
				error: function(xhr, textStatus, error, $form) {
					$form.find(":submit").prop("disabled", false).removeClass("btn-before-submit");
					$form.trigger("error.shopec.ajaxSubmit", arguments);
				},
				complete: function(xhr, textStatus, $form) {
					$form.trigger("complete.shopec.ajaxSubmit", arguments);
				}
			};
		...
		$().ready(function() {
			...
			// 用户注册/登录
			$document.on("registered.shopec.user loggedIn.shopec.user", function(event, currentUser) {
				$.setCurrentUser(currentUser);
				$.removeCurrentCartQuantity();
			});
			
			// 用户注销
			$document.on("loggedOut.shopec.user", function(event, currentUser) {
				$.removeCurrentUser();
				$.removeCurrentCartQuantity();
				$.removeSpreadUser();
			});
			...
			if ($.getCurrentCartQuantity() == null) {
				$.getCurrentCart().done(function(data) {
					...
					var beforeEvent = $.Event("before.shopec.setCurrentCartQuantity");
					...
					$document.trigger("complete.shopec.setCurrentCartQuantity", quantity);
				});
			}
			
			// 加入/修改/移除/清空购物车
			$document.on("success.shopec.addCart success.shopec.modifyCart success.shopec.removeCart success.shopec.clearCart", function(event, data) {
				var quantity = data.quantity != null ? data.quantity : 0;
				var beforeEvent = $.Event("before.shopec.setCurrentCartQuantity");
